...
async function seed() {
  ...
  // Workouts
  const w1 = new Workout({ title: 'Quick HIIT', description: 'Short high-intensity workout', exercises: [{ name: 'Burpees', reps: 10, sets: 3 }], durationMinutes: 20, createdBy: alice._id });
  const w2 = new Workout({ title: 'Morning Yoga', description: 'Gentle flow', exercises: [{ name: 'Sun Salutation', reps: 5, sets: 2 }], durationMinutes: 30, createdBy: carol._id });
  await w1.save();
  await w2.save();
